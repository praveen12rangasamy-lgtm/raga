...
<head>
  ...
  <title>Sarees Collection — Raga Boutique</title>
  <!-- Favicon -->
  <link rel="icon" type="image/png" href="images/raga_favicon.png">
  <meta name="description" content="Browse our collection of handwoven sarees including Banarasi silk, Kanjivaram, lightweight organza, and cotton drapes.">
...
<body class="bg-brand-cream text-brand-charcoal min-h-screen flex flex-col">
    ...
    <div class="flex flex-col lg:flex-row gap-8">
      ...
      <aside id="filter-sidebar" class="hidden lg:block lg:w-64 bg-white p-6 border border-brand-gold/15 sticky top-28 max-h-[calc(100vh-8rem)] overflow-y-auto shadow-sm z-30 flex-shrink-0">
        ...
        <div class="space-y-6 text-xs">
          ...
          <!-- Filter: Dynamic Categories & Sub-Categories Tree -->
          <div>
            <h4 class="font-serif text-xs uppercase tracking-wider font-bold text-brand-burgundy mb-3">Categories & Craft</h4>
            <div id="sidebar-categories-tree" class="space-y-3 text-xs text-brand-charcoal/95">
              <!-- Dynamically populated via JS -->
            </div>
          </div>

          <hr class="border-brand-gold/15">

          <!-- Price Range -->
          <div>
            <h4 class="font-serif text-xs uppercase tracking-wider font-bold text-brand-burgundy mb-3">Price Range</h4>
            <div class="space-y-2">
      ...
      <section id="catalog-top" class="flex-grow flex flex-col min-h-[500px]">
        ...
        <!-- Sarees Grid -->
        <div class="grid grid-cols-2 md:grid-cols-3 gap-2.5 sm:gap-6" id="sarees-listing-grid">
  ...
  <script>
    ...
    function initializeQueryParameters() {
      ...
      // Active Category: From URL ?category=... or default 'sarees'
      const urlCat = params.get("category");
      let activeCatId = 'sarees';
    ...
    function renderCatalog(shouldScroll = false) {
      const grid = document.getElementById("sarees-listing-grid");
    ...
    function resetFilters() {
      ...
      const currentCat = (activeFilters.categories && activeFilters.categories.length > 0) ? activeFilters.categories[0] : 'sarees';
